...
class Isotope_Posts {
     ...
     * @var      string
     */
    protected $plugin_slug = 'itsisotope';
    ...
    private function __construct() {
        ...
        require_once( ITS_DIR . '/admin/views/settings.php' );
    ...
    public function enqueue_styles() {
        wp_register_style($this->plugin_slug . '-bootstrap-styles', plugins_url('assets/css/bootstrap.min.css', __FILE__), array(), self::VERSION);
        wp_register_style($this->plugin_slug . '-plugin-styles', plugins_url('assets/css/public.css', __FILE__), array(), self::VERSION);

        global $post;

        if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'itsisotope')) {
    ...
    public function isotope_loop($atts) {
        ...
        if ($load_css == 'true') {
            wp_enqueue_style($this->plugin_slug . '-bootstrap-styles');
            wp_enqueue_style($this->plugin_slug . '-plugin-styles');
        }
        ...
        if ($filter_menu == 'yes' && taxonomy_exists($filter_by)) {
            ...
            if ($count > 0) {
                echo '<ul id="filters" class="nav nav-tabs">';
                echo '<li><a href="#" data-filter="*">' . __('All', 'isotope-posts-locale') . '</a></li>';
        ...
        <div class="container">
            <div id="iso-loop" class="row masonry-grid">
                <?php while ($isoposts->have_posts()) : $isoposts->the_post(); ?>
                    <div class=" <?php if ($filter_menu == 'yes' && taxonomy_exists($filter_by)) { $terms = get_the_terms($isoposts->post->ID, $filter_by); if (!empty($terms)) { foreach ($terms as $term) { echo $term->slug . ' '; } } } ?>iso-post text-center">
                        <?php
    ...
    /**
     * Register the shortcode "[itsisotope]".
     *
     * @since    2.0.0
     */
    public function register_shortcode() {
        add_shortcode('itsisotope', array($this, 'isotope_loop'));
    }
